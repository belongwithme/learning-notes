...
站点正文（包括标题、代码和图表）禁用文本选中、复制、剪切、右键菜单和拖拽；阅读时拦截 Ctrl/Cmd+A、C、X，并关闭代码块的复制按钮。搜索框、输入框和可编辑区域保留正常操作，正文链接、折叠内容和图表缩放仍可使用。

这是页面交互限制，不能彻底保护公开内容：截图、网页源码、直接请求页面或关闭 JavaScript 等方式仍可能取得正文，不应将其作为保密或访问控制措施。

`npm run build` 会清理 Astro 内容缓存后重新构建，确保代码块等渲染配置应用到已有文章。修改此类配置后，如果本地开发页面仍显示旧代码块，停止开发服务并删除 `.astro/data-store.json` 后再启动。
