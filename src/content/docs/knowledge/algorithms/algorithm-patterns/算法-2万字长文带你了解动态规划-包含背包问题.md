...
有n件物品和一个最多能背重量为w 的背包。第i件物品的重量是weight[i]，得到的价值是value[i] 。每件物品只能用一次，求解将哪些物品装  
 入背包里物品价值总和最大。  
 · ![在这里插入图片描述](./assets/127416130/839c5cb39252aa0a6fdd65dc.png)  
 题目：  
 背包最大重量为4。
...
1.我们先去确定dp数组的定义以及下标的定义  
 ...
 ![在这里插入图片描述](./assets/127416130/cf516dbd2b4ed8f21bcb58df.png)  
 2.确定递推公式  
 明确两点，[状态]和[选择]。  
...
3.dp数组的初始化  
 ...
 ![在这里插入图片描述](./assets/127416130/13adeb0e9aaed8940ea449ab.png)  
...
![在这里插入图片描述](./assets/127416130/a57678eddf75c74a4746fc82.png)  
...
4.确定遍历顺序  
 ...
 ![在这里插入图片描述](./assets/127416130/da762093f52f9e43a2958722.png)  
 **但其实，在背包问题里，两个for循环先后顺序是非常有讲究的**，后面我们用一维数组的时候，到时候再探究。

5.举例推导dp数组  
 来看一下对应的dp数组的数值，如图：  
 ![在这里插入图片描述](./assets/127416130/34855dbbbc0418524bebf2c8.png)  
 结果为dp[2][4]。  
...
5.举例推导dp数组  
 ![在这里插入图片描述](./assets/127416130/4b9d55c553a2153685870e92.png)  
...
基本思路：  
 ...
 情况一：去头  
 ![在这里插入图片描述](./assets/127416130/64a709cdbef91d2e6f126dce.png)  
 第二种情况：去尾  
 ![在这里插入图片描述](./assets/127416130/604be7c91084d6861ba24145.png)  
 第三种：去头尾  
 ![在这里插入图片描述](./assets/127416130/21143b3d4c143d16b419f420.png)  
...
* 举例推导dp数组  
   ![-](./assets/127416130/9fce99c65ca6edf90c752db1.png)  
   代码如下：
...
* 递推公式  
   ...
   **这里不要写dp[i]与dp[j]+1进行比较，我们要取dp[j]+1的最大值，对这点不清晰很容易写错。血的教训**  
   ![在这里插入图片描述](./assets/127416130/ae1c0261ff921b70c046a079.png)
* dp数组的初始化  
...
* 举例推导dp数组  
   ![在这里插入图片描述](./assets/127416130/47b2c8565f86358c8b343489.png)
...
* 举例推导dp数组  
   ![在这里插入图片描述](./assets/127416130/6e16039f34b026ad2fa420f0.png)  
   代码如下：
...
* 外层循环A，内存循环B（反过来也可以）。
* 举例推导dp数组  
   ![在这里插入图片描述](./assets/127416130/c2738ca1d02cbd4ef60558cc.png)  
   代码如下：
...
* 由递推公式可以看出，有三个可以退出dp[i][j]  
   ![在这里插入图片描述](./assets/127416130/98c2dfb2cf078c55d00597bd.png)  
   那么在计算左下时，要经过从前到后，从上到下。
* 举例推导dp数组  
   ![在这里插入图片描述](./assets/127416130/a8b58642db3e96cee86ca409.png)  
...
* 举例推导dp数组  
   ![在这里插入图片描述](./assets/127416130/0bbc1c3c3879755c1a7cba02.png)  
   代码如下：
...
* dp数组如何初始化  
   ...
   ![在这里插入图片描述](./assets/127416130/0b25ce4d2038a47929a0a643.png)  
   如果要是定义的dp[i][j]是以下标i为结尾的字符串s和以下标j为结尾的字符串t，初始化就比较麻烦了。  
   dp[i][0] 表示以下标i-1为结尾的字符串，与空字符串的相同子序列长度，所以为0. dp[0][j]同理。
* 确定遍历顺序  
   从递推公式可以看出dp[i][j]都是依赖于dp[i-1][j-1]和dp[i][j-1]，那么遍历顺序应该是从上到下，从左到右。  
   ![在这里插入图片描述](./assets/127416130/09ecc08c79e062eb66623427.png)
* 举例推导dp数组  
   ![在这里插入图片描述](./assets/127416130/e00e3e1c8a1fd5ba38b0296b.png)  
   代码如下：
...
* 举例推导dp数组  
   ![在这里插入图片描述](./assets/127416130/6c6e8d86b40bff8652ba50ab.png)  
   代码如下：
...
* 确定遍历顺序  
   ...
   ![在这里插入图片描述](./assets/127416130/496a6469e6ee6beddcf33827.png)  
   所以我们是从下到上，从左到右遍历，这样保证dp[i+1][j-1]都是经过计算的。  
   **有的代码是先遍历列，再遍历行，也是一个道理，都保证了dp[i+1][j-1]经过计算。**
* 举例推导dp数组  
   ![在这里插入图片描述](./assets/127416130/ce58ac418510cdfee17285a3.png)  
...
* 确定递推公式  
   ...
   ![在这里插入图片描述](./assets/127416130/66f2a7ae6d5451fa4b6450e4.png)  
   ...
   ![在这里插入图片描述](./assets/127416130/c4ed7d86c5068ea45e8b70e7.png)
* 确定遍历顺序  
   从递推公式dp[i][j] = dp[i + 1][j - 1] + 2 和 dp[i][j] = max(dp[i + 1][j], dp[i][j - 1]) 可以看出，dp[i][j]是依赖于dp[i + 1][j - 1] 和 dp[i + 1][j]。  
   那么我们回到矩阵角度来说，我们从下到上，从左到右遍历。  
   ![在这里插入图片描述](./assets/127416130/dabf61e87b3ba07c1e0323fb.png)
* 举例推导dp数组  
   ![在这里插入图片描述](./assets/127416130/a9fa3a8df87b6437edc95904.png)  
   代码如下：
