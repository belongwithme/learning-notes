...
单链表大概长这样：  
![在这里插入图片描述](./assets/63ce5338f5477489606f30d8.png)  
很容易看出来大概是以头节点领着的一条线，首尾没有串起来。  
那么环状链表就是把首尾串起来而已，大概长这样：  
![在这里插入图片描述](./assets/3a94959fb4cf90a89cfe93c4.png)  
...
1：添加链表  
首先展示一个初始情况：  
![在这里插入图片描述](./assets/8da4b0d69cfc37e9dfaa53d7.png)  
这个时候我们设置指针，将它自己首尾相连，同时增加一个辅助指针，它用于后面添加指针时，first指向最新的，而此时辅助指针留在之前最后的节点，可以连接新的节点。  
![在这里插入图片描述](./assets/57cf97e7b1f996df94a4dd9c.png)  
现在假设增加一个链表：  
a:初始情况  
![在这里插入图片描述](./assets/7ca461666a291efe785396f3.png)  
b:随后指针开始指向构建：  
1：（过程开始)first指针一直指向头链表，方便新链表指向first指针  
2：辅助指针首先将指向的本链表连接新链表（过程q），随后移动到新链表中（过程w)  
走到过程q时:  
![在这里插入图片描述](./assets/a575d065b49a801dcf777738.png)  
走到过程w时:  
![在这里插入图片描述](./assets/abd1dc7227521afa91f4dae3.png)  
链表结构代码如下：
...
结果：  
![在这里插入图片描述](./assets/3bedb447f94a8ed4937dd0b0.png)
...
结果：  
![在这里插入图片描述](./assets/282943f09bd536401063deb0.png)
