...
![在这里插入图片描述](./assets/69d956bb96f5c1a33e43b7c4.png)  
 那如何解决呢?肯定是不能放任不管的呀!
...
针对不同的隔离级别,并发事务可能发生的现象也会不同,如下图:  
 ![在这里插入图片描述](./assets/c5003ea3f640911d07221609.png)  
...
* DELETED\_BIT(1byte)  
   ...
   ![在这里插入图片描述](./assets/d12179eb53c4ca0781f2cf99.png)  
...
1. 有一个事务插入进来,如下,隐式主键为1,事务ID和回滚指针，我们假设为NULL.  
    ![在这里插入图片描述](./assets/2acf46435d4a7af6b09ccd0d.png)
...
* 事务提交后，释放锁  
   最后变为如下:  
   ![在这里插入图片描述](./assets/898d29ce153bb178f2e85b0b.png)
...
* 事务提交，释放锁  
   最后结果如下:  
   ![在这里插入图片描述](./assets/c7d52d6d9df9a3664424abd7.png)  
...
![在这里插入图片描述](./assets/912be13f40e68b86c01c4610.png)
...
![在这里插入图片描述](./assets/e5e9ec434b306d9a0cd0e8d7.png)
...
我们吧Read View 的图拿出来看:  
 ![在这里插入图片描述](./assets/dea6e16fd166371348fe48de.png)  
 上面提到的能回滚,是依靠与隐藏列,如下图:  
 ![在这里插入图片描述](./assets/f0cc8737b5e7da29b51c1131.png)
...
流程图如下:  
 ![在这里插入图片描述](./assets/0d1fde06fa95e2c855efde72.png)
...
假设事务 A （事务 id 为51）启动后，紧接着事务 B （事务 id 为52）也启动了，那这两个事务创建的 Read View 如下：  
 ![在这里插入图片描述](./assets/3e716849c6028298a12bce7a.png)  
...
接着，事务 A 通过 update 语句将这条记录修改了（还未提交事务）,将小林的余额改成 200 万，这时 MySQL 会记录相应的 undo log，并以链表的方式串联起来，形成版本链,如下图:  
 ![在这里插入图片描述](./assets/359a1f4b395a7666a4299557.png)
...
![在这里插入图片描述](./assets/f5c33f6a2a8ba556480d04ab.png)
...
由于隔离级别是「读提交」，所以事务 B 在每次读数据的时候，会重新创建 Read View,此时事务 B 第三次读取数据时创建的 Read View 如下：  
 ![在这里插入图片描述](./assets/29001fdad9c924ebec8a7882.png)  
