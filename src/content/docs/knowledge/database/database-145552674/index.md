...
![在这里插入图片描述](./assets/49b1c75641d5e98bd65be966.png)  
...
执行后获取的结果如下:  
 ![在这里插入图片描述](./assets/dac5303964da5e9d98f33ed3.png)  
 从上图可以看到，共加了两个锁，分别是：
...
* **当条件值对应的记录在表中时**：
  ...
  + **如果是 `<=` 查询条件**：当扫描到终止范围记录时，**`next-key` 锁不会退化成间隙锁**，直接锁住该记录。  
     下面我们详细解释.我们目前表中数据如下:  
     ![在这里插入图片描述](./assets/e95a8c31101c2a718d34d518.png)
...
执行结果如下:  
 ![在这里插入图片描述](./assets/8e7b049d7444ebeff4e0e8ec.png)  
 我们可以看到加的是两个行级锁:
...
执行结果如下:  
 ![在这里插入图片描述](./assets/8b02fb4dc1d68ac58b491ab2.png)  
 可以看到了加了三个锁:
...
现在我们修改一下表的数据,修改后如下:![在这里插入图片描述](./assets/1d97a24ea5a542b2fe5c6e10.png)
...
分析锁如下:  
 ![在这里插入图片描述](./assets/144e067e88e4eb08f44d9918.png)  
 可以看出加了三个锁:
...
分析锁得出:  
 ![在这里插入图片描述](./assets/0ed6625108ee6acb5eefde28.png)  
 可以看出加了两个锁:
...
分析锁得到如下:  
 ![在这里插入图片描述](./assets/9d1c6c13629e92946e2e07ec.png)  
 可以看出获取的锁:
...
表格数据如下:  
 ![在这里插入图片描述](./assets/e14f6793b2c9ef420f6ab76f.png)
...
分析锁如下:  
 ![在这里插入图片描述](./assets/3ec1b377eaa572c9d6f9cabd.png)  
 加锁流程如下:
...
![在这里插入图片描述](./assets/fb173d37e1182218927a8d2a.png)  
...
执行结果如下:  
 ![请添加图片描述](./assets/d8eaccf0f4d3665df149a0f8.png)  
 我们可以看到是加了三个锁:
...
非唯一索引和主键索引的范围查询的加锁也有所不同.  
 ...
 ![请添加图片描述](./assets/ff7def4496e9388ece181f18.png)  
 我们可以看到是加了五个锁:
