...
注意这里还有一个指向下一个哈希表节点的指针，我们知道哈希表最大的问题是存在哈希冲突，如何解决哈希冲突，有开放地址法和链地址法。这里采用的便是链地址法，通过next这个指针可以将多个哈希值相同的键值对连接在一起，用来解决哈希冲突。  
 ![在这里插入图片描述](./assets/93dcfd45e146012aa3c307ae.png)
...
哈希表实际上是一个数组，数组里每一个元素就是一个哈希桶。  
 ...
 ![在这里插入图片描述](./assets/63ae5c32c5faef1b00a1c88f.png)
...
所谓的链式哈希，就是用一个链表把映射到 Hash 表同一桶中的键给连接起来。下面我们就来看看 Redis 是如何实现链式哈希的，以及为何链式哈希能够帮助解决哈希冲突。  
 ...
 ![在这里插入图片描述](./assets/5bf9f50c4b2e0969cd4a17da.png)  
...
![在这里插入图片描述](./assets/d7b338c4e503f8ba10fc9279.png)  
...
![在这里插入图片描述](./assets/385dfba08735a47b73ed9f40.png)  
...
rehash 的触发条件跟\*\*负载因子（load factor）\*\*有关系。  
 ![在这里插入图片描述](./assets/b152180e75997e016f87ff9e.png)  
...
* dictAddorFind：直接调用 dictAddRaw。  
   ...
   ![在这里插入图片描述](./assets/7a91e687f4ebb3577511f9c0.png)  
