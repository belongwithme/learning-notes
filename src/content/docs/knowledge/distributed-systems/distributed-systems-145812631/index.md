...
Redis 的主从架构中，由于主从模式是读写分离的，如果主节点（master）挂了，那么将没有主节点来服务客户端的写操作请求，也没有主节点给从节点（slave）进行数据同步了。  
 ![在这里插入图片描述](./assets/c2aecf4cdb5a780edfdc4fe6.png)  
...
哨兵其实是一个运行在特殊模式下的 Redis 进程，所以它也是一个节点。  
 ...
 ![在这里插入图片描述](./assets/872f27b812944932cdc18607.png)  
 那么如何负责这些事情呢?

#### 监控

哨兵会每隔 1 秒给所有主从节点发送 PING 命令，当主从节点收到 PING 命令后，会发送一个响应命令给哨兵，这样就可以判断它们是否在正常运行。  
 ![在这里插入图片描述](./assets/0b8d07cda784bf1c208a5ce0.png)  
...
客观下线只适用于主节点。  
 ...
 ![在这里插入图片描述](./assets/55977bb8cbbcc723653fe84a.png)  
...
假设有三个哨兵。当哨兵 B 先判断到主节点「主观下线后」，就会给其他实例发送 is-master-down-by-addr 命令。  
 ...
 ![在这里插入图片描述](./assets/4cd6d1f59faf933fc7a1b6a5.png)  
...
选举出了哨兵 leader 后，就可以进行主从故障转移的过程了.  
 如图:  
 ![在这里插入图片描述](./assets/25b9f407cb75384aae687f6e.png)  
 主从故障转移操作包含以下四个步骤：
...
如果在第一轮考察中，发现优先级最高的从节点有两个，那么就会进行第二轮考察，比较两个从节点哪个复制进度。  
 ...
 ![在这里插入图片描述](./assets/1e7c3d4abce9b8c949d67118.png)  
...
到这里，选主的事情终于结束了。简单总结下:  
 ![在这里插入图片描述](./assets/0c06eedba40837f4db98aeb7.png)  
 ...
 ![在这里插入图片描述](./assets/27409067e5ece80c9d7872c0.png)  
...
客户端和哨兵建立连接后，客户端会订阅哨兵提供的频道。  
 ...
 ![在这里插入图片描述](./assets/dfe1284a30474ed74fd2dcd0.png)
...
在我第一次搭建哨兵集群的时候，当时觉得很诧异。因为在配置哨兵的信息时，竟然只需要填下面这几个参数，设置主节点名字、主节点的 IP 地址和端口号以及 quorum 值。  
 ...
 ![在这里插入图片描述](./assets/8a2138f42cbba5da34c12939.png)  
...
**哨兵集群会对「从节点」的运行状态进行监控，那哨兵集群如何知道「从节点」的信息？**  
 ...
 ![在这里插入图片描述](./assets/08e7b6900076c77a88288307.png)  
