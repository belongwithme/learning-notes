...
**redis 内部使用文件事件处理器 `file event handler`，它是单线程的，所以redis才叫做单线程模型。它采用IO多路复用机制同时监听多个 socket，将产生事件的 socket 压入内存队列中，事件分派器根据 socket 上的事件类型来选择对应的事件处理器进行处理。**![在这里插入图片描述](./assets/145711338/037c1ddb3d62f8c070853261.png)
...
6. 事件分派器找到相关联的命令回复处理器，由命令回复处理器对 socket01 输入本次操作的一个结果，比如 ok，之后解除 socket01 的 AE\_WRITABLE 事件与命令回复处理器的关联。  
    整个流程如下图:  
    ![在这里插入图片描述](./assets/145711338/38e637e94c3a6f014e086c97.png)
...
但是，使用多线程后，没有良好的系统设计，可能会出现如下图所示的场景，增加了线程数量，前期吞吐量会增加，再进一步新增线程的时候，系统吞吐量几乎不再新增，甚至会下降！  
 ![在这里插入图片描述](./assets/145711338/50c97f859ff5905f51a154a3.png)  
 多线程引入后需要考虑到:
...
似的，当 Redis 通过 recv() 从一个客户端读取数据时，如果数据一直没有到达，Redis 也会一直阻塞在 recv()。  
 ![在这里插入图片描述](./assets/145711338/ac0c1c27045c8fcacdd09bbc.png)  
 阻塞的原因由于使用传统阻塞 IO ，也就是在执行 read、accept 、recv 等网络操作会一直阻塞等待。如下图所示：  
 ![在这里插入图片描述](./assets/145711338/a45f4130245fa4e561475e45.png)
...
Socket 网络模型的非阻塞模式设置，主要体现在三个关键的函数调用上，如果想要使用 socket 非阻塞模式，就必须要了解这三个函数的调用返回类型和设置模式。接下来，我们就重点学习下它们。  
 ...
 ![在这里插入图片描述](./assets/145711338/ac697a47a9c86d71947d375c.png)  
...
select/epoll 提供了基于事件的回调机制，即针对不同事件的发生，调用相应的事件处理器。所以 Redis 一直在处理事件，提升 Redis 的响应性能。  
 ![在这里插入图片描述](./assets/145711338/bee8c7e2fbae0937f8191000.png)  
 那么，回调机制是怎么工作的呢？其实，select/epoll 一旦监测到 FD 上有请求到达时，就会触发相应的事件。  
 ![在这里插入图片描述](./assets/145711338/4f6e80082cf6c2d818de90cd.png)  
