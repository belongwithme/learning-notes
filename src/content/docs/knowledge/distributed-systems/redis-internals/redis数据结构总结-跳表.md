...
链表在查找元素的时候，因为需要逐一查找，所以查询效率非常低，时间复杂度是O(N)，于是就出现了跳表。  
 ...
 跳表示例图如下,层级为 3 的跳表::  
 ![在这里插入图片描述](./assets/145731773/726442ab2012d4306f66dd18.png)  
...
* **层的创建规则**  
   ...
   ![在这里插入图片描述](./assets/145731773/3420de427dd0de339a7b8923.png)

### 前进指针

每个层都有一个指向表尾方向的前进指针（level[i].forward 属性）， 用于从表头向表尾方向访问节点。  
 下图中虚线代表前进方向  
 ![在这里插入图片描述](./assets/145731773/2ec3122370e331c57a5688f3.png)
...
* 指向 NULL 的所有前进指针的跨度都为 0 ， 因为它们没有连向任何节点。  
   ...
   举个例子:  
   ![在这里插入图片描述](./assets/145731773/bb3d9e9a53698f3fa7b7f9fb.png)  
   ...
   ![在这里插入图片描述](./assets/145731773/44968e4178d0dd3d8f25134d.png)
...
* 节点的成员对象（obj 属性）是一个指针， 它指向一个字符串对象， 而字符串对象则保存着一个 SDS（简单动态字符串） 值。  
   ...
   ![在这里插入图片描述](./assets/145731773/bc115b60d4b91e63c3dc58c6.png)

### 跳跃表

仅靠多个跳跃表节点就可以组成一个跳跃表,如下图:  
 ![在这里插入图片描述](./assets/145731773/261aa9b1c26136a0b35f5181.png)  
 但是不太好管理,所以Redis通过使用一个 zskiplist 结构来持有这些节点， 程序可以更方便地对整个跳跃表进行处理， 比如快速访问跳跃表的表头节点和表尾节点， 又或者快速地获取跳跃表节点的数量（也即是跳跃表的长度）等信息，如下图:  
 ![在这里插入图片描述](./assets/145731773/afa1d43ec6878f74dbe90732.png)  
 zskiplist结构的定义如下:
...
举例:下图有个 3 层级的跳表  
 ![在这里插入图片描述](./assets/145731773/b4842c8aef05a3231ca0ae17.png)  
...
跳表的相邻两层的节点数量的比例会影响跳表的查询性能。  
 ...
 ![在这里插入图片描述](./assets/145731773/5fa37527cf63c557b66debd7.png)  
 如果想要查询节点 6，那基本就跟链表的查询复杂度一样，就需要在第一层的节点中依次顺序查找，复杂度就是 O(N) 了。所以，为了降低查询复杂度，我们就需要维持相邻层结点数间的关系.  
 **跳表的相邻两层的节点数量最理想的比例是 2:1，查找复杂度可以降低到 O(logN)。**  
 下图的跳表就是，相邻两层的节点数量的比例是 2 : 1。  
 ![在这里插入图片描述](./assets/145731773/b9bcce49f577424507fea5b5.png)  
 那么什么时候增加层数呢?  
