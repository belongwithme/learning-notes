...
我之前就是习惯复制整个项目目录来保存不同版本（同时改名+备份时间）。  
 ...
 ![在这里插入图片描述](./assets/c51caf87810d96094dd35bea.png)
...
如果是在不同系统上开发者如何协同工作呢？  
 ...
 ![在这里插入图片描述](./assets/65a0c36a810533cbd27b68ea.png)  
 好处：  
...
根据上述的情况，分布式控制系统（Distrubuted Version Control System，简称DVCS）面世了。  
 ...
 ![在这里插入图片描述](./assets/0fdb1ee7f74a65158ba17658.png)
...
* 直接记录快照，而非差异比较  
   ...
   ![在这里插入图片描述](./assets/45a049a336b78a5f5fbb4513.png)  
   ...
   ![在这里插入图片描述](./assets/b62cd91e13391adc90761314.png)
* 近乎所有操作都是本地执行  
...
* 文件的三种状态  
   ...
   ![在这里插入图片描述](./assets/81f6e133edde40912c170b7e.png)  
...
这里我们先进到E盘中，然后调用git init，效果如下：  
 ![在这里插入图片描述](./assets/960b3b10759664ab7c97c5f1.png)  
 .git里面的目录结构：  
 ![在这里插入图片描述](./assets/de322f11fc8434edcd4b5fb2.png)
...
这会在当前目录下创建一个名为Spring-shiro的目录，其中包含了一个.git目录，用来保存下载下来的所有版本记录，然后从中取出最新版本的文件拷贝。  
 ...
 ![在这里插入图片描述](./assets/4ff6e4d4055e4700ff65d8f7.png)  
 ![在这里插入图片描述](./assets/5cc63c9dd380c4eb895bd66c.png)
...
注意：工作目录下面的所有文件都不外乎两种状态——已跟踪或未跟踪。  
 ...
 如下图所示：  
 ![在这里插入图片描述](./assets/a82c7600d4e4ae5292df5297.png)
...
如果你看见这样的输出：  
 ![在这里插入图片描述](./assets/b9515d5228ec9255290ec79a.png)  
...
接着我们用vim创建一个新文件 Wang，保存退出后运行git status会看见该文件出现在未跟踪文件列表中：  
 ![在这里插入图片描述](./assets/c0e951490d3e2f900432a4aa.png)  
...
此时再运行 git status命令，会看到Wang文件已被跟踪，并处于暂存状态：  
 ![在这里插入图片描述](./assets/eb378e2f6ec0019841454c54.png)  
...
现在我们修改下之前跟踪过的文件Wang，再次运行status命令，会看到这样的状态报告：  
 ![在这里插入图片描述](./assets/77e8144de934798cf69146a3.png)  
...
创建一个新文件benchmarks.rb,编辑后把最新版本暂存下来：  
 ![在这里插入图片描述](./assets/26b5dd34c8a331e85c58f5c5.png)

再次修改benchmarks.rb不暂存,运行status命令将会看到：  
 ![在这里插入图片描述](./assets/7ddb5672e143341ad36523ea.png)  
 （这里请忽略Wang）  
 要查看尚未暂存的文件更新了哪些部分，不加参数直接输入git diff：![在这里插入图片描述](./assets/3bdf1160be54f9320ae161fc.png)  
 此命令比较的是工作目录中当前文件和暂存区域快照之间的差异，也就是**修改之后还没有暂存起来的变化内容**。

* 如果你想看已经暂存起来的文件和上次提交时的快照之间的差异，可以用 `git diff --cached`命令:  
   ![在这里插入图片描述](./assets/54b529c8040f143992bb5cee.png)
...
这种方式会启动文本编译器**以便输入本次提交的说明**。  
 编译器会显示类似下面的文本信息：  
 ![在这里插入图片描述](./assets/37bcedc9306580fecdf31000.png)  
 可以看到，默认的提交消息包含最后一次运行git status 的输出，放在注释里。

另外也可以用 -m参数后跟提交说明的方式，在一行命令中提交更新：  
 ![在这里插入图片描述](./assets/780cdcd1900246a9f50e45a4.png)  
...
尽管使用暂存区域的方式可以精心准备要提交的细节，但有时候这么做略显繁琐。  
 ...
 ![在这里插入图片描述](./assets/305a16e7f2148e9e87d5489b.png)  
...
实际上，即使此时查看状态信息，也会明白无误地看到关于重命名操作的说明：  
 ![在这里插入图片描述](./assets/9880db59141b8205ab6fd0d6.png)  
...
的远程库，Git默认使用这个名字来标识你所克隆的原始仓库：  
 ![在这里插入图片描述](./assets/e68059f5754c145e8cc64443.png)  
 可以在这个命令后面加“-v”选项（verbose的简写，取首字母），显示对应的克隆地址：  
 ![在这里插入图片描述](./assets/e86184ac640c1d940fe3e945.png)  
 如果有多个仓库，则会全部取出。
...
![在这里插入图片描述](./assets/86667c75e9b6abcc9f3ffbab.png)
...
git fetch demo
```
![在这里插入图片描述](./assets/remote-043223db4a6c9924.png)
