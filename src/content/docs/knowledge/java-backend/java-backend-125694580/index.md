...
![在这里插入图片描述](./assets/5ee3917de485e523e8d1ecb8.png)  
 代码实现（实现继承关系)：  
 ![在这里插入图片描述](./assets/709f7555310d772d83aa49f8.png)
...
首先给出两个图  
 ![在这里插入图片描述](./assets/e0c417debe0029e060bb88f4.png)![在这里插入图片描述](./assets/bcba19c61fa341711ee58369.png)  
...
* 要实现不可修改的AbstractList。  
   ...
   实现如下![在这里插入图片描述](./assets/ec21533b08136567f8112609.png)  
   如果直接去用，则报错（语法也不通过），如图：  
   ![在这里插入图片描述](./assets/a09a0c2c8e2698760437ca80.png)
* 实现可修改的AbstractList。  
   必须另外重写set(int, E)方法（否则抛出UnsupportedOperationException ）。
* 实现可变大小的AbstractList。
* 必须另外重写add(int, E)和remove(int)方法。举个add的例子，remove同理，感兴趣可以试一试，如图：![在这里插入图片描述](./assets/263be055e95f567b3369a5ed.png)  
   ...
   ![在这里插入图片描述](./assets/2c79d70135bb23c959a02e11.png)

2. List：实现List接口操作规范，增删遍历等操作。
3. RandomAccess：提供随机访问功能。  
    解读：ist集合中用的较多的就是ArrayList 和 LinkedList 两个类，这两者作比较你可以发现，**ArrayList有这个接口，但是LinkedList没有**，如图：![在这里插入图片描述](./assets/ad36c560e821f5cf77b9aaec.png)  
    ![在这里插入图片描述](./assets/c6d70d72027f47e30913b8b0.png)  
    现在我们去看看这个RandomAccess，进入后发现接口里面什么都没有= =。  
    ![在这里插入图片描述](./assets/8f5fabb29be710b958945031.png)  
...
5. Serializable: 提供可序列化功能。  
    ...
    咳咳，先看一下这个接口发现什么都没有  
    ![在这里插入图片描述](./assets/3b6502fc1f1709d95e1d93c2.png)  
...
接着运行外边的方法ensureExplicitCapacity。  
 ...
 ![在这里插入图片描述](./assets/9272bb228e4c69cd4d9f66e1.png)  
 接着进入无参构造方法，且elementData对象是DEFAULTCAPACITY\_EMPTY\_ELEMENTDATA  
 ![在这里插入图片描述](./assets/3126e13b952a560bfba3160e.png)  
 接着进入第一次add的过程如下：  
 ![在这里插入图片描述](./assets/2f2453ee269b21d2a486c1ae.png)  
 首先要执行 ensureCapacityInternal(size + 1)，注意此时的参数size=0,e=1![在这里插入图片描述](./assets/a8184acf06149fc2b1edc9c3.png)接着进入ensureCapacityInternal(int minCapacity)，这里们很清楚，这里的minCapacity是1  
 ![在这里插入图片描述](./assets/6ece0a86111dec83e987b945.png)  
 然后执行ensureExplicitCapacity嵌套的方法calculateCapacity(Object[] elementData, int minCapacity)  
 进入之后首先判断我们构造函数的对象是不是无参的。  
 ![在这里插入图片描述](./assets/d683c206c216b4c37fd44825.png)  
 这里因为我们是无参的构造，所以进入的if判断，注意这里的max参数的值，DEFAULT\_CAPACITY=10，minCapacity=1，所以我们将要返回的无疑是DEFAULT\_CAPACITY=10.  
 ![在这里插入图片描述](./assets/a2506032c531e919a9493f25.png)  
 这里我们看到了minCapacity变为了10.  
 接着我们要进入if判断，**我们要扩容的值-原数组大小，如果>0，则进行扩容。**  
 ![在这里插入图片描述](./assets/db0d19d22d8fce3d0ee949f2.png)  
 然后我们在上面已经介绍过，这里简单看一下执行过程里面的参数大小和改变。  
 刚进入时：  
 ![在这里插入图片描述](./assets/24f7b5bdc7511a49784dbbf8.png)  
 执行扩容算法后：因为原先老数组大小为0，所以扩容1.5倍依旧为0  
 ![在这里插入图片描述](./assets/bebf4ea9b1c565ea4eafa379.png)  
 下面执行判断逻辑，第一个判断：newCapacity - minCapacity < 0，新容量大小是否大于预设容量大小，这里明显0-10<0,所以进入方法块：  
 ![在这里插入图片描述](./assets/728c21b343896e9b3fc6ecb9.png)  
 我们可以看到，新数组容量变为了10，之后执行第二个判断：newCapacity - MAX\_ARRAY\_SIZE > 0，目的是看一下新扩容大小是否超过最大标准，这里显然不大于，所以进行下一步：  
 ![在这里插入图片描述](./assets/268f45b3f4ae4c6ce176f854.png)  
 接下来是数组的拷贝转移：  
 首先进入  
 ![在这里插入图片描述](./assets/0c668f82374777f37636639f.png)  
 接着进入：注意参数  
 ![在这里插入图片描述](./assets/e00611a3583c56932c72cd44.png)  
 这里先比较数组类型，然后创建数组。然后进行拷贝，最后返回拷贝好的数组copy。  
 ![在这里插入图片描述](./assets/dedb354635b8365ddb549342.png)  
 之后一步步返回到开始的地方：  
 ![在这里插入图片描述](./assets/b3d7e4ac3f31f2d126e8004b.png)  
 执行代码后add方法完成：  
 ![在这里插入图片描述](./assets/4b95c1a7e28f829aae93d2e0.png)  
