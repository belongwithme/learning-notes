...
![在这里插入图片描述](./assets/1479087478468878e6265375.png)  
...
![在这里插入图片描述](./assets/46f31c735c228897b361a825.png)

单核CPU下，线程实际还是串行执行的，**操作系统中有一个组件叫做任务调度器，将cpu的时间片分给不同的线程使用  
 只是cpu在线程间的切换非常快，人感觉是同步运行的，但是本质是串行**

并行：![在这里插入图片描述](./assets/989addc9d56f86dad6ace3fa.png)多核CPU下，每个核都可以调度运行线程，这时候线程可以并行，同一时间做多件事。

#### 异步调用

方法的调度有两种：同步和异步  
 首先介绍同步：需要等待结果的返回才会继续向下运行  
 ![在这里插入图片描述](./assets/99d4759ebba21b6d78ef4f42.png)  
 异步：不需要等待结果，就能继续运行  
 ![在这里插入图片描述](./assets/e640a7aabaa7581ae08f10a5.png)  
...
任务管理器查看  
 ![在这里插入图片描述](./assets/5071d641927e0dd626c75984.png)  
...
栈就是**CPU寄存器里某个指针指向的一片内存区域**，每个Java线程都有一个私有的Java虚拟机栈，创建线程的同时栈也被创建。  
 ![在这里插入图片描述](./assets/181af7a594a280c1460a24ad.png)

##### 栈帧

一个JVM栈由许多帧组成，称为“栈帧”  
 ![在这里插入图片描述](./assets/89bf430441d741c1e16bcd13.png)  
...
保存的状态包括：  
 ...
 ![在这里插入图片描述](./assets/e4ae7fe63167533576b4c9fa.png)
...
被创建的`Thread`对象直接调用重写`run`方法时，`run`方法是在主线程中被执行的，而不是在我们创建的线程中执行，如果想到在创建的线程中执行`run`方法，需要使用`Thread`对象的`start`方法。  
 ...
 ![在这里插入图片描述](./assets/677adcc4d0446c4244c6700a.png)
...
4. 哪个线程内调用`join`方法，就等待哪个线程结束，然后再去执行其他线程  
    ![在这里插入图片描述](./assets/b9afc9dcb2ef89778fc32b16.png)
...
打断阻塞：  
 ...
 ![在这里插入图片描述](./assets/663e4f22fab6291adbc02175.png)
...
![在这里插入图片描述](./assets/5076b8602b5190b11c570cf3.png)  
...
![在这里插入图片描述](./assets/9b2bf91c2e9e9cb09943319e.png)  
 ![在这里插入图片描述](./assets/b7bbe7cebe72e0112cba8d2a.png)
