...
我们需要理解几个概念，以便我们更好的了解HashMap。  
 ...
 ![在这里插入图片描述](./assets/7d2746549717d547153d3a31.png)

#### 继承关系

![在这里插入图片描述](./assets/feb781cdd4ef69d0f08df24f.png)  
 代码如下：
...
![在这里插入图片描述](./assets/5da27bcfc690aa27587eeacd.png)  
...
t：保存hash数组的table  
 ![在这里插入图片描述](./assets/1fd3c629a456d74b42a1ac6d.png)  
 index：数组的索引
 ...
 **有个问题还需要思考：它到底怎么把键加入到keyset里面的？**  
 ...
 ![在这里插入图片描述](./assets/c2b9a55baecb1dbec10bcce1.png)  
...
我们之前看keySet类的构造函数，发现只有默认构造函数，如果不是在keySet构造函数里把HashMap的可以加入进来，那么可能是KeySet的父类构造函数中加入进来：  
 ![在这里插入图片描述](./assets/2ba4dd41e28d97b8183c8d52.png)  
 ![在这里插入图片描述](./assets/c64bd5cc4b6975279f434756.png)然而我们通过上图可以看到，都是空实现。。。  
 这部分真的卡了我好久= =！  
...
**细心的你可能发现了：调试代码中，key没有加入到set中，idea是如何显示出ks有元素的？**  
 ...
 ![在这里插入图片描述](./assets/36ea30c0ddd05875c8773322.png)
...
6. 插入成功，判断hashmap的size是否超过threshold的值，超过则扩容。  
    ![在这里插入图片描述](./assets/be3a39423726db1d4c50c6a5.png)
