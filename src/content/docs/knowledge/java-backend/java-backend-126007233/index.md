...
一段代码块内如果存在对共享资源的多线程读写操作，称**这段代码块**为临界区。  
 举个例子：  
 ![在这里插入图片描述](./assets/cbe558473ba928289729f694.png)  
 ![在这里插入图片描述](./assets/8fa61fc65662802ba23b0037.png)  
 ...
 **原子性，可见性，有序性**（后边我们会详细解释）  
 ...
 ![在这里插入图片描述](./assets/d397df76f0ea609d7b91432d.png)  
 ...
 ![在这里插入图片描述](./assets/0f1a66087b384353f9cdfe58.png)  
...
![在这里插入图片描述](./assets/b14f2b0ea53c2f8ade115ec3.png)

##### 同步静态方法上：锁作用于类

![在这里插入图片描述](./assets/60a12b7ca3d4469fd1835e9d.png)

##### 同步类关键字：锁作用于类

![在这里插入图片描述](./assets/458a692f9e66ae697f16f661.png)

##### 同步this，锁作用于本身

![在这里插入图片描述](./assets/8461cd20191634a33b86e025.png)
...
![在这里插入图片描述](./assets/58dfd4a9dfe1e1dc562a5e65.png)  
 它的字节码（注意：局部变量和静态变量不一样，局部变量只有一步iinc）  
 ![在这里插入图片描述](./assets/6390368fe6c9cef3e1010602.png)局部变量是线程安全的——每个方法都在对应线程的栈中创建栈帧，不会被其他线程共享，如下图：

![在这里插入图片描述](./assets/b98877f452c4821aa307c5cd.png)  
...
线程不安全的情况：如果调用的对象被共享，且执行了读写操作，则线程不安全  
 ![在这里插入图片描述](./assets/4f306ab6339dc2b5ef8ba270.png)  
 如果是局部变量，则会在堆中创建对应的对象，不存在线程安全问题：  
 ![在这里插入图片描述](./assets/59c9d9c897dee2b110dba088.png)
...
![在这里插入图片描述](./assets/58435b4fad9091c635f96dd6.png)  
 它们每个方法都是原子（都被加上了`synchronized`），但是它们多个方法的组合不是原子的，所以可能出现线程问题，如下图：  
 ![在这里插入图片描述](./assets/c40690c9f49e6d2c2f355c1c.png)
...
![在这里插入图片描述](./assets/a650fd148d8e8f078b27e97e.png)对于同步方法，通过反编译我们可以看出，JVM采用`ACC_SYNCHRONIZED`标记符来实现同步，如下图所示：  
 ![在这里插入图片描述](./assets/8908b8ea044db5b7e0dd3916.png)  
 **注意**：
...
![在这里插入图片描述](./assets/1ba92d3707c51365941faa0f.png)  
 JVM采用`monitorenter、monitorexit`两个指令来实现同步，如下图：  
 ![在这里插入图片描述](./assets/140c0855edfb1dddc0b1f2bf.png)
...
我们在这里只列举最重要的去说明  
 ![在这里插入图片描述](./assets/2439bc09fd358aea899b284f.png)
...
![在这里插入图片描述](./assets/69eccff76af186bbbddfa0f4.png)

#### 数组对象的Java对象头

![在这里插入图片描述](./assets/ce97a113297888cfa8b870fe.png)

#### Mark Word结构

32位虚拟机  
 ![在这里插入图片描述](./assets/dc4451eeb1eff9809b4c496c.png)

![在这里插入图片描述](./assets/f36a81a4043d6705eb3fa79d.png)
