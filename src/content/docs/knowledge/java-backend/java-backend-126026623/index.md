...
如果一个线程在给一个对象加轻量级锁时，cas替换操作失败（因为此时其他线程已经给对象加了轻量级锁），此时该线程就会进入锁膨胀过程。  
 ...
 ![在这里插入图片描述](./assets/f5a0bb0f91299733a75ad16e.png)  
 流程

1. 将对象头的Mark Word改为Monitor地址，并且状态改为10（重量级锁）
2. 将该Thread\_1线程放入EntryList中，并进入阻塞状态(blocked)  
    ![在这里插入图片描述](./assets/9a7500658e2a64f503f56a6f.png)
 ...
 **优点**  
 ...
 ![在这里插入图片描述](./assets/e7c80c9d803836aa8e64c0d0.png)  
 重量级锁竞争，还可以使用自旋来进行优化  
 如果当前线程自旋成功（即这时候持锁线程已经退出了同步块，释放了锁），  
 这时当前线程就可以避免阻塞。

如果自旋失败，则堵塞。  
 ![在这里插入图片描述](./assets/94dda911fa4096f7edbff4c2.png)  
...
用于优化轻量级锁重入，原因在于：**轻量级锁在没有竞争时**，每次重入（该线程执行的方法再次锁住该对象）操作仍需要cas替换操作，这样会使性能降低。  
 ![在这里插入图片描述](./assets/f15c6094902446c586bcdba5.png)  
...
在第一次cas时会将线程的Id写入对象Mark Word中。  
 ...
 才会对创建的对象设置为偏向状态。  
 ![在这里插入图片描述](./assets/609ae2e49690642d95de6571.png)
...
![在这里插入图片描述](./assets/3f06aee0393ceeba36419528.png)  
