...
JVM是Java Virtual Machine（Java虚拟机）的缩写，它是一个虚构出来的计算机，是通过在实际计算机上仿真模拟各种计算机功能来实现的。  
 ...
 展示一个小黑盒来看一下：  
 ![在这里插入图片描述](./assets/fcc118b508bca625188e88de.png)  
 我们再来开里面的黑盒  
 JVM对字节码进行解释，翻译成特定底层平台匹配的机器指令并运行。（后面会解释为什么要翻译成字节码）  
 ![在这里插入图片描述](./assets/6bb30a2a7d74848281aeff09.png)  
 如果把中间JVM细节隐藏，那么就会成这样  
 ![在这里插入图片描述](./assets/afbde62453dce4a66968a5a7.png)  
...
JVM是不可以直接运行Java代码的，所以Java代码必须要先编译成字节码文件，JVM才能正确识别代码转换后的指令并将其运行。  
 ![在这里插入图片描述](./assets/d53d594d181b3c909f719444.png)  
...
简单过目一下，有个印象，后面我们会详细聊聊：  
 ![在这里插入图片描述](./assets/097cc0387ce19abb9a96f76d.png)
...
我们以JDK中的HotSpot虚拟机为例，从虚拟机以及底层硬件两个角度，来聊一聊Java虚拟机具体怎么运行Java字节码。  
 ...
 ![在这里插入图片描述](./assets/1475f459759436cb293682b1.png)  
...
从硬件角度：Java字节码无法直接执行。因此，JVM需要将字节码翻译成机器码。  
 ...
 ![在这里插入图片描述](./assets/13bc67e51ee4964d74d9cf73.png)  
 解释执行的优势：无需等待编译  
 即时编译优势：实际运行速度更快  
...
JVM内存区域最粗略的划分可以分为堆和栈，当前，按虚拟机规范，可以划分以下几个区域：  
 ![在这里插入图片描述](./assets/af1a80b46811b72cf9277dfc.png)  
...
多线程在一个特定时间段内只会执行其中某一个线程方法，CPU会不停的做任务切换，这样必然会导致经常中断或恢复。为了能准确的记录各个线程正在执行的当前字节码指令地址，所以为每个线程都分配了一个PC寄存器，每个线程都独立计算，不会相互影响。  
 ...
 ![在这里插入图片描述](./assets/4459d86e822442fc104f6ef5.png)
...
**线程私有**  
 ...
 下图可以帮助理解：  
 ![在这里插入图片描述](./assets/f6493e875f90ea6fc10cd639.png)
...
* 一些附加信息  
   附下图：  
   ![在这里插入图片描述](./assets/806af130f264a6781c6f1a46.png)
...
首先不要慌，我们一点点看，先不要去关注常量池里的东西，我们先关注下图画红线的代码：  
 ![在这里插入图片描述](./assets/05459e07a6381f5e1e8fe6ac.png)  
 我们来看一下a()方法里的一条字节码指令：
...
1.每一个栈帧内部都包含一个指向**运行时常量池**中该栈帧所属方法的引用。包含这个引用的目的就是为了支持当前方法的代码能够实现动态链接（Dynamic Linking）。  
 ...
 ![在这里插入图片描述](./assets/b9bd69587823ad671cd57863.png)  
...
![在这里插入图片描述](./assets/95e10c315f9447521335caa5.png)  
 ![在这里插入图片描述](./assets/e0d4503a63e9eada9b0a0adc.png)  
 ![在这里插入图片描述](./assets/95e10c315f9447521335caa5.png)
...
![在这里插入图片描述](./assets/2328ae7380e2841a3cca604b.png)  
 ![在这里插入图片描述](./assets/bda92ed486d748c7ceccfd0b.png)
...
我们先看一个图片  
 ![在这里插入图片描述](./assets/74d3a8516884f5bd416ab0bc.png)  
 它的具体做法是：  
...
为了高效的垃圾回收，虚拟机把堆内存逻辑上分为了三块区域（分代的唯一理由就是优化GC性能），贴个图看一下：  
 ![在这里插入图片描述](./assets/b3d53eb4c9469c51e86ca4c2.png)  
 简单解释一下三大块内容：  
...
年轻代是所有新对象创建的地方。当填充年轻代时，会执行垃圾收集（Minor GC）。  
 ...
 ![在这里插入图片描述](./assets/5437f91f50ece749fc038695.png)  
...
![在这里插入图片描述](./assets/ad8bf86374b9b5989b134e8e.png)
...
如果一个对象为Hello，已经经历了15次Minor GC还存活在Survivor空间中。那么他会将转移到老年代，这个15是年龄阈值，可以根据-XX:MaxTenuringThreshold来设置，默认是15。  
 ...
 ![在这里插入图片描述](./assets/10017efbbb903531394f98d9.png)
...
除了上面所说的达到年龄阈值，还有一种方式可以进入老年代，那就是动态年龄判断：在Survivor空间中相同年龄所有对象大小的总和>Survivor空间的一半，年龄>=该年龄的对象可以直接进入老年代。  
 ...
 ![在这里插入图片描述](./assets/520e11c1c245cec1359324ec.png)
...
我们来张图总结一下：  
 ![在这里插入图片描述](./assets/560943cc90d4e587979ce06d.png)  
 总结:
...
是各个线程共享的内存区域，用于存储已被JVM加载的类型信息，常量，静态变量，JIT编译后的代码缓存等数据。  
 ...
 JVM关闭后方法区即被释放。  
 ![在这里插入图片描述](./assets/7faeccc650724712bc2ba80f.png)

##### 方法区，永久代，元数据哪个才是张麻子

![在这里插入图片描述](./assets/1b75e3b902673f30bf7beb75.png)  
 有的内存结构图有方法区，有的是永久代，元数据，到底什么情况？  
 ![在这里插入图片描述](./assets/c118f8b27405f7536d976186.png)  
...
大致区域浏览图：  
 ![在这里插入图片描述](./assets/964803d7e33b9a4e0826394b.png)  
 《深入理解 Java 虚拟机》书中对方法区（ Method Area) 存储内容描述如下：它用于存储已被虚拟机加载的类信息、常量、静态变量、即时编译器编译后的代码缓存等。  
 ![在这里插入图片描述](./assets/d6d2f564494fb8b052b63771.png)  
 我们一个个看这些内容：
...
我们知道一个Java源文件中的类，接口，编译后产生一个字节码文件。  
 ...
 ![在这里插入图片描述](./assets/a7dabf0f1fa7d16eea96bda7.png)  
...
在动态链接的时候用到的就是运行时常量池。  
 ...
 ![在这里插入图片描述](./assets/a3e2cafa6c9154d9b342565c.png)  
...
![在这里插入图片描述](./assets/92fd5b49e2f7d3ef4d273230.png)  
 **字符串常量池的内存布局**

在JDK1.7之前运行时常量池逻辑包含字符串常量池存放在方法区，此时HotSpot虚拟机对方法区的实现为永久代。  
 ![在这里插入图片描述](./assets/3de4daae6a94c271669d167e.png)  
 在JDK1.7字符串常量池和静态变量被从方法区拿到了堆中，运行时常量池剩下的还在方法区，也就是HotSpot中的永久代。  
 ![在这里插入图片描述](./assets/57d4b07f5e02ede1489c5a93.png)  
 在JDK8 时，HotSpot移除了永久代改为元空间，这时候字符串常量池还在堆中，运行时常量池还在方法区，只不过方法区的实现从永久代改为了元空间。  
 ![在这里插入图片描述](./assets/1070df16be6150d349002543.png)
