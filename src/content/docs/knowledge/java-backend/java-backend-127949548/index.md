...
先看一下内存区域  
 ![在这里插入图片描述](./assets/61c59e369947cbd6e3b1fb6a.png)
...
上面那种是计数法，因为会造成循环依赖问题，所以不被广泛使用。  
 ...
 ![在这里插入图片描述](./assets/9e89b112fc610086c3bd803b.png)  
...
强弱分代假说奠定了垃圾收集器的设计原则：  
 ...
 ![在这里插入图片描述](./assets/09f52a349e9ecd07be5b067e.png)
...
标记清除算法是基础垃圾回收算法，分为两部分：  
 ...
 2.把这些垃圾拎出来清理掉。  
 ![在这里插入图片描述](./assets/6313589050268df12977675c.png)
...
标记-复制算法基于“半区复制”算法：  
 ...
 ![在这里插入图片描述](./assets/35b14fbc65067a06924c7ea0.png)### 标记-整理算法  
 ...
 ![在这里插入图片描述](./assets/f540e1ad1e8c8fe9f99701ec.png)
...
* 并发（Concurrent）：并发描述的是垃圾收集器线程与用户线程之间的关系，说明同一时间垃圾收集器线程和用户线程都在运行。但由于垃圾收集器线程会占一部分系统资源，所以程序的吞吐量依然会受到一定影响，除了CMS和G1之外，其他垃圾收集器都是以串行方式执行。  
   下图中的连线代表了可以配合使用。  
   ![在这里插入图片描述](./assets/b9d0be3dc4c2c531d3f3ee36.png)
...
最基础，历史最悠久的收集器，进行垃圾回收时，必需暂停其他所有的工作线程，直到收集结束。  
 ...
 ![在这里插入图片描述](./assets/4a1b4cda38a1be373620d117.png)

#### Serial Old收集器

它是Serial收集器的老年代版本，同样是一个单线程收集器，采用标记-整理算法，主要用于给客户端模式下的HotSpot虚拟机使用：  
 ![在这里插入图片描述](./assets/44c6e726e259ad8d9bf6cfed.png)

#### ParNew收集器

Serial收集器的多线程版本，可以使用多条线程进行垃圾回收：  
 ![在这里插入图片描述](./assets/3839209db9815866d7031476.png)

#### Paralled Old 收集器

它是Parallel Scavenge收集器的老年代版本，支持多线程并发收集，采用标记-整理算法实现：  
 ![在这里插入图片描述](./assets/41f72be37ebf91b644f2f95f.png)
...
它是一种以获取最短回收停顿时间为目标的收集器，基于标记-清除算法实现，整个收集过程分为以下四个阶段：  
 ...
 ![在这里插入图片描述](./assets/ed4a151e2fe82f5e7e625276.png)  
 缺点：
...
![在这里插入图片描述](./assets/a4c5a38ca6ae55b462455eb6.png)  
 G1把堆划分为多个大小相等的独立区域，新生代和老年代不再物理隔离。  
 （H代表了大对象）  
 ![在这里插入图片描述](./assets/e3a1e9c997932266b0b17885.png)
...
* 筛选回收：首先对各个Region中的回收价值和成本进行排序，根据用户所期望的GC停顿时间来制定回收计划。此阶段其实也可以做到与用户程序一起并发执行，但是因为只回收一部分Region，时间是用户可控制的，而且停顿用户线程将大幅度提高收集效率。  
   ![在这里插入图片描述](./assets/b171f3eb585a1027cbd83dcb.png)
