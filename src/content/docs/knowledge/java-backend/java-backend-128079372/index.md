...
直到今天，或许还有相当一部分程序员认为Java虚拟机执行Java程序是一件理所当然天经地义的事情。但是Java技术发展之初，设计者就考虑过其他语言运行在Java虚拟机之上的可能性。  
 ...
 因此，有一些Java语言本身无法有效支持的语言特性并不代码在字节码也无法有效表达出来，这为其他程序语言实现一些有别于Java的语言特性提供了发挥空间。![请添加图片描述](./assets/e687d95812542851a07af57a.jpg)
...
* 表  
   ...
   ![请添加图片描述](./assets/adcdae78c6a49e9bb2d33ea0.jpg)  
...
这里我们先一个个介绍，最后列一个总和和例子来帮助理解。  
 这里给一个例图：  
 ![在这里插入图片描述](./assets/560c842c74800cf058ae1caa.png)
...
常量池中每一项常量都是一个表，最初常量表中共有11种各不相同的表结构数据，后来为了更好地支持动态语言调用，额外增加了4种动态语言相关的常量，为了支持Java模块化系统（Jigsaw），又加入了CONSTANT\_Module\_info和CONSTANT\_Package\_info两个常量，所以截至JDK 13，常量表中分别有17种不同类型的常量。  
 ...
 17种常量类型具体含义如下图所示：  
 ![请添加图片描述](./assets/3f7476a715aa0296c1546277.jpg)  
 ![请添加图片描述](./assets/5fd36ccbb8bd1a7ad84c2ebf.jpg)  
...
我们再来看一下这个图:  
 ...
 我们来简单看一看这个类型的结构：  
 ![请添加图片描述](./assets/f2f2189031a4bd62ac4cbb72.jpg)  
 tag是标志位，它用于区分常量类型。  
 ...
 ![请添加图片描述](./assets/73d7efe682cd51201e4298b9.jpg)  
 length值说明了这个UTF-8编码的字符串长度是多少字节，它后面紧跟的长度为length字节的连续数据是一个使用UTF-8缩略编码表示的字符串。  
 本例中字符串的length值为0x001D，长29字节。![请添加图片描述](./assets/d72fda1d8f634a4a6eb67094.jpg)  
...
final 
等等。  
 ![请添加图片描述](./assets/2c37b26d4a40fe7e370f44d6.jpg)  
...
demo3是一个普通的类，不是接口、枚举、注解或者模块，被public关键字修饰但没有被声明为final和abstract，并且它使用了JDK 1.2之后的编译器进行编译，因此它的ACC\_PUBLIC、ACC\_SUPER标志应当为真，而ACC\_FINAL、ACC\_INTERFACE、ACC\_ABSTRACT、ACC\_SYNTHETIC、ACC\_ANNOTATION、ACC\_ENUM、ACC\_MODULE这七个标志应当为假，因此它的access\_flags的值应为：0x0001|0x0020=0x0021。![请添加图片描述](./assets/27cbe0635a722270134a6bf6.jpg)
...
我们来看一下字段表的结构：  
 ![请添加图片描述](./assets/3aa5feb8bba4a6a379047b4b.jpg)  
 字段修饰符放在access\_flags项目中，它与类中的access\_flags项目是非常类似的，都是一个u2的数据类型，其中可以设置的标志位和含义如下图所示：  
 ![请添加图片描述](./assets/f0e4ab1e77fbab039ba46663.jpg)  
 很明显，由于语法规则的约束：
...
描述符的作用是用来描述字段的数据类型，方法的参数列表（包括数量，类型以及顺序）和返回值。根据描述符的规则，基本数据类型（byte，char，double，float，int，long，short，boolean）以及代表无返回值的void类型都用一个大写字符来表示，而对象类型则用字符L加对象的全限定名来表示，如下图：  
 ![请添加图片描述](./assets/eea18703418a0cfadbe16fb2.jpg)  
...
Class文件存储格式中对方法的描述与对字段的描述采用了几乎完全一致的方式，方法表的结构如同字段表一样，依次包括访问标志（access\_flags），名称索引（name\_index）,描述符索引（descriptor\_index），属性表集合（attributes）几项，如下图所示：  
 ![请添加图片描述](./assets/538c585291eb4c5990572121.jpg)  
 ...
 如下图：  
 ![请添加图片描述](./assets/fb499ae0c364e4786e0739b4.jpg)  
...
答：方法里的代码，经过Javac编译器编译成字节码指令后，存放在方法属性表集合中一个名为“Code”的属性里面，属性表后面会聊到。这里给个图看一下：  
 ![在这里插入图片描述](./assets/9d321c24f658428b3c3e0706.png)  
 我们以之前一共Class文件为例，对方法表集合进行分析。![请添加图片描述](./assets/be856b453aa3d119cb57f690.jpg)  
...
Class文件，字段表，方法表都可以携带自己的属性表集合，以描述某些场景专有的信息。  
 ...
 下图简单看一看：  
 ![请添加图片描述](./assets/65b1c4b3b21005f33a8d3365.jpg)  
 ![请添加图片描述](./assets/21e51e56ad91e8bcbf1d9f7a.jpg)  
 对于每一个属性，它的名称都要从常量池中引用一个CONSTANT\_Utf8\_info类型的常量来表示，而属性值的结构则是完全自定义的，只需要通过一个u4的长度属性去说明属性值所占用的位数即可。  
 应该满足下图的结构：  
 ![请添加图片描述](./assets/b549239093bd528eb8615620.jpg)
...
Java程序方法体里面的代码经过Javac编译器处理之后，最终变为字节码指令存储在Code属性内。  
 ...
 ![请添加图片描述](./assets/981710a37739c87e1bd1ecdc.jpg)
...
![请添加图片描述](./assets/b37e10999a67133fcd9480fc.jpg)  
...
在字节码指令之后的是这个方法的显示异常处理表（简称“异常表”），异常表对于Code属性来说并不是必须存在的，如果存在异常表，格式如下图所示：  
 ![请添加图片描述](./assets/78ffdd9b3e762977e9e54afe.jpg)  
...
列举出方法中可能抛出的受查异常，也就是方法描述时在throws关键字后面列举的异常，结构如下：![请添加图片描述](./assets/98600be93b5b0dfa905cf122.jpg)  
...
用于描述Java源码行号与字节码行号（字节码的偏移量）之间的对应关系。  
 结构属性如下图：![请添加图片描述](./assets/396f38f051882c59aca3f47a.jpg)  
...
LocalVariableTable属性用于描述栈帧中局部变量表的变量与Java源码中定义的变量之间的关系，它也不是运行时必需的属性，但默认会生成到Class文件中。  
 ![请添加图片描述](./assets/f9b495044f7e155aff469bec.jpg)  
...
其中local\_variable\_info项目代表了一个栈帧与源码中的局部变量的关联，结构如下所示：  
 ![请添加图片描述](./assets/7693217946dbe3d4aec8eca3.jpg)  
...
通知JVM自动为静态变量赋值。只有被Static关键字修饰的变量（类变量）从可以使用这项数量。  
 属性结构如下：  
 ![请添加图片描述](./assets/79e820bae9de00a9576d9972.jpg)  
...
记录内部类与宿主类之间的关联。如果一个类中定义了 内部类，那编译器将会为它以及它所包含的内部类生成InnerClasses属性。  
 ...
 ![请添加图片描述](./assets/10f3f6847d50bfcc2affe2c3.jpg)
...
Deprecated和Synthetic两个属性属于标志类型的布尔属性，只存在有和没有的区别，没有属性值的概念。  
 ...
 它们的结构如下：  
 ![请添加图片描述](./assets/bcbf851c6a70da2b9df035fa.jpg)
...
记录泛型的签名信息。  
 ...
 属性结构如下表：  
 ![请添加图片描述](./assets/0e65383c161c0bd02821e244.jpg)  
...
* MethodParameters属性  
   作用是记录方法的各个形参名称和信息。  
   结构如下：  
   ![请添加图片描述](./assets/0924b0ac9fa20823c433e71f.jpg)  
   其中，引用到的parameter结构如下表：  
   ![请添加图片描述](./assets/28e40c04ca40c744bf1ae095.jpg)  
