...
对于资源来说，并不是所有的资源都需要认证和授权，有些公共的资源是可以直接访问的，所以我们要对资源有一个分类。  
 ![请添加图片描述](./assets/b5f42643069c3e24197feecb.png)
...
新构建一个module模块，项目结构如下：  
 ![请添加图片描述](./assets/395929582670ecaded7b3d3f.png)  
 其中config包下的代码如下所示：
...
它接受一个`AuthenticationSuccessHandler`，那么我们来看看这个：  
 它的结构图如下：  
 ![请添加图片描述](./assets/25743665d10fd6bb09cefe4c.png)  
...
首先我们在登陆页面输入错误的信息，然后再进入到`attemptAuthentication`中，如下图：  
 ![请添加图片描述](./assets/7a20fbfbbcbe7d64bfa0f6f1.png)  
 然后我们进入到最后一行代码里的`authenticate`方法：  
 ![请添加图片描述](./assets/a521bbc304b8ff045de4b6c2.png)  
...
。  
 ...
 ![请添加图片描述](./assets/3f607fc109ec552823881b7d.png)  
 走到for循环的第一个if判断时，因为支持待验证的Authentication对象的类，所以不进去方法体，当走到下图时：![请添加图片描述](./assets/7cf9d1f05fad844d6951f5e1.png)  
...
下面走debug的流程，执行到第一个判断，发现不是从缓存中拿到的用户数据：  
 ![进入第一个循环](./assets/e55ec375022ce9ff6cb7e49e.png)  
 然后进入try，执行retrieveUser，从数据源拿用户数据，发现拿不到（因为我们输入的是错误的数据），执行下面的catch方法：![请添加图片描述](./assets/1db2bc5d442dc57a4a920ba2.png)  
 然后继续向下执行，看最后报错是哪个：  
 ![请添加图片描述](./assets/5944f1362f2fcddacdb495f0.png)  
...
好的我们继续回来，抛出`BadCredentialsException`后，接着回调到provider为`DaoAuthenticationProvider`中：![请添加图片描述](./assets/fac93b86f3203699bd67f71b.png)  
 执行完catch中的方法后,跳出for循环，因为只有一个provider：  
 ![请添加图片描述](./assets/98d1d3237f554e1ab29f30a9.png)  
 接着往下走，到if条件中发现，都不符合：  
 ![请添加图片描述](./assets/d43f8e78650544dd63a6436b.png)  
 到第三个if判断中，还是不符合：  
 ![请添加图片描述](./assets/053ef8ebf4ba57b35e232e61.png)  
 接着进入下一个if判断，我们此时lastException不为空，所以不符合：  
 ![请添加图片描述](./assets/e75b10321d8a445be10bc2ad.png)  
 走到最后一个if条件时，发现我们的parentException为空，符合条件：

![请添加图片描述](./assets/ad0ec1bc43b1d98cd1462667.png)  
...
接着抛出错误：  
 ![请添加图片描述](./assets/89505057dca7de349d2132af.png)  
 最后回到provider为`AnonymousAuthenticationProvider`，执行完catch里面的方法后，进入if判断，此时result为空，不符合：  
 ![请添加图片描述](./assets/a3a298dbb62cc5409f7f54bb.png)  
 进入下一个判断，这里lastException不为空，所以不符合：  
 ![请添加图片描述](./assets/c83dd37a0274195c8ac2aaa6.png)  
 最后一个判断，此时parentException不为空，不符合：

![请添加图片描述](./assets/c83dd37a0274195c8ac2aaa6.png)  
 最后抛出错误：  
 ![请添加图片描述](./assets/075fa0952acea1a2cf3228c0.png)  
 ...
 ![请添加图片描述](./assets/023a72cb6982e67116abadd7.png)  
...
我们执行到最后一步，进入到身份验证失败处理器方法里面：  
 ![请添加图片描述](./assets/0d97d0f84f081e2eabfdae06.png)  
...
debug走一下里面的流程,刚进入时，注意里面的默认失败url，此时默认失败url不为null，不符合：  
 ![请添加图片描述](./assets/a77bff78bc6366595e71d3f3.png)  
 然后执行saveException，将异常保存到请求属性中：  
 ![请添加图片描述](./assets/6efe9ea08d3832e7428c6aaa.png)  
 它里面的内容如下：
...
接着执行下一个if条件，因为forwardToDestination为false，也就是没有设置为跳转转发，所以不会为true，不符合，然后直接进入到else里面，进行重定向转发。![请添加图片描述](./assets/7e4ff504daa82af486212912.png)
...
最后实现效果：  
 ![在这里插入图片描述](./assets/9476159d07a5bee2ed5af6ef.png)
...
它的结构如下图：  
 ![请添加图片描述](./assets/4bfaee34998f5bae8499eada.png)
...
实际上 SecurityContextHolder 中存储是 SecurityContext，在  
 ...
 ![在这里插入图片描述](./assets/93e23fb5f0edc41ce144aa87.png)
...
![在这里插入图片描述](./assets/af0cd1ea72b37524331cac65.png)
...
* 多线程情况下获取⽤户数据  
   ...
   ![在这里插入图片描述](./assets/12e19ca89702f5721f95f34c.png)
...
结果如下：  
 ![请添加图片描述](./assets/c4411f3a9d1c3aaecc47c40f.png)
